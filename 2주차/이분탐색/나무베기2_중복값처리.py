...
def index_check(tree):
    return len(tree) == len(set(tree)) # 나무 리스트의 길이와 중복값을 제거한 나무 리스트의 길이가 같으면 True, 아니면 False를 반환한다.

# 입력
while True:
    N, M = map(int, input().split()) # 나무의 수 N, 집으로 가져가려고 하는 나무의 길이 M
    tree = list(map(int, input().split()))  # `N`개의 나무를 리스트로 받는다.

    if len(tree) != N: # 입력한 나무의 수가 N개와 다르면
        print(f"{N}개의 나무 길이를 입력해야 합니다. 다시 입력해주세요.") # 다시 입력하라고 출력하고
        continue # 다시 입력 받기

    # 이진 탐색 사용을 위한 준비
    tree.sort() # 이진 탐색을 수행하기 위해서는 반드시 정렬해야함.

    if index_check(tree) == False: # 만약 나무 리스트에 중복값이 있다면
        print("중복된 나무의 길이가 있습니다. 다시 입력해주세요.") # 중복된 나무의 길이가 있다고 출력하고
        continue # 다시 입력 받기
    else: # 중복된 나무의 길이가 없다면
        break # 그냥 넘어간다.
